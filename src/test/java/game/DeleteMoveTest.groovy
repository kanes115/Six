...
class DeleteMoveTest extends Specification {
    ...
    def "Delete move removes cards in given positions if cards are removable and the same"(){
        given:
        CasualPosition pos1 = new CasualPosition(new Card(Color.CLUBS, Face.ACE), targetFace)
        CasualPosition pos2 = new CasualPosition(new Card(Color.CLUBS, Face.ACE), targetFace)
    ...
    def "Delete move does not delete different cards"(){
        given:
        CasualPosition pos1 = new CasualPosition(new Card(Color.CLUBS, Face.THREE), targetFace)
        CasualPosition pos2 = new CasualPosition(new Card(Color.CLUBS, Face.ACE), targetFace)
        Move m = new DeleteMove(pos1, pos2, State.PREPARING)
        when:
        !m.execute()
    ...
    def "Delete move does not delete cards if at least one is not removable"(){
        given:
        CasualPosition pos1 = new CasualPosition(new Card(Color.CLUBS, Face.SIX), targetFace)
        CasualPosition pos2 = new CasualPosition(new Card(Color.CLUBS, Face.ACE), targetFace)
        Move m = new DeleteMove(pos1, pos2, State.PREPARING)
        when:
        !m.execute()
